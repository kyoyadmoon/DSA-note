...
const HORIZONTAL_HEIGHT = 420;
const VERTICAL_HEIGHT = 760;

function randomValues(length: number): number[] {
...
export function TreeAlgorithmRunnerClient({
  slug,
  initial,
  highlightedLines,
  meta,
  ...
  const handleStepChange = useCallback(
    (idx: number) => {
      setStepDirection(idx === currentStep ? 0 : idx > currentStep ? 1 : -1);
      setCurrentStep(idx);
  ...
  const handleRandomize = useCallback(() => {
    setInput(randomValues(initial.length));
    setCurrentStep(0);
    setIsPlaying(false);
  }, [initial.length]);
